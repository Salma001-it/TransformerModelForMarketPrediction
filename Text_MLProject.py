...
- checkTheSimilarityEmb:
    in order to retrive past event
...
class NewsEmbeddingPipeline:
    ...
    def download_text(self,file_path,company_name,head_name,ticker_name):
        ...
        text = load_dataset("SelmaNajih001/FinancialNewsAndSummary")
        text=text["train"].to_pandas()
    ...
    def checkTheSimilarityEmb(self,df_text_with_embedding,query):
        ...
        #Apply L2 normalization to the embedding. Going back to our initial dataset, we had that each row was the news for a day. With the embedding we have each row representing the news. So the L2 normalization should be applied for each row. That way we obtain the L2 norm
