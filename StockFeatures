...
class StockSequenceBuilder:
    def __init__(self, ticker="TSLA", start="2016-01-01", end="2024-06-30",
        ...
        corr=Data.corr() #Calculate the correlation between all the tickers
        corr=corr[ticker].sort_values(ascending=False).head(20) #Select our ticker in order to find the most correlated with the one selected
        corr=corr.reset_index()["Ticker"].to_list()
        self.correlated=corr #It contains the list of the top 20 correlated tickers
    ...
    def compute_features(self):
        ...
        # Download the prices of the correleted stocks and compute their log return 
        corr_prices = yf.download(self.correlated, start=start_dt, end=end_dt)["Close"]
        corr_returns = np.log(corr_prices / corr_prices.shift(1))
        df["Corr_Index_Return"] = corr_returns.mean(axis=1) #The mean of the correlated stock for day t
